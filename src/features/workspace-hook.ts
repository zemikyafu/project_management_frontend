...
export function useCreateWorkspace(companyId:UUID){
    ...
    return useMutation({
        mutationFn:(workspaceData:WorkspaceCreateFormValues)=>WorkspaceService.createWorkspace(workspaceData,companyId),
        onSuccess:()=>{
...
export function useUpdateWorkspace(companyId:UUID){
    ...
    return useMutation({
        mutationFn:(workspaceData:WorkspaceUpdateFormValue)=>WorkspaceService.updateWorkspace(workspaceData,companyId),
        onSuccess:()=>{
            queryClient.invalidateQueries({queryKey:workspaceKey(companyId)});
        }
    });
